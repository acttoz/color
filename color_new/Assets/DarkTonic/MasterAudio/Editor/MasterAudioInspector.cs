...
public class MasterAudioInspector : Editor {
    ...
    public override void OnInspectorGUI() {
        ...
        if (sounds.areGroupsExpanded) {
			...
			EditorGUILayout.BeginHorizontal();
			GUILayout.Space(10);
			var labelText = sounds.showGroupCreation ? "Collapse Group Creation" : "Expand Group Creation";
            ...
            if (groups.Count == 0) {
                ...
                for (var l = 0; l < filteredGroups.Count; l++) {
                    ...
                    if (Application.isPlaying) {
                        ...
                        if (GUILayout.Button(string.Format("[{0}]", groupVoices), EditorStyles.toolbarButton)) {
                    ...
                    if (aList.isExpanded) {
                        ...
                        for (var j = 0; j < aList.MusicSettings.Count; j++) {
                            ...
                            if (aSong.isExpanded) {
                                ...
                                GUI.color = Color.white;
                                var exp = EditorGUILayout.BeginToggleGroup("Song Started Event Settings", aSong.songStartedEventExpanded);
                                if (exp != aSong.songStartedEventExpanded) {
                                    UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle expand Song Started Event Settings");
                                ...
                                if (sounds.useGaplessPlaylists) {
                                    ...
                                    exp = EditorGUILayout.BeginToggleGroup("Song Changed Event Settings", aSong.songChangedEventExpanded);
                                    if (exp != aSong.songChangedEventExpanded) {
                                        UndoHelper.RecordObjectPropertyForUndo(ref isDirty, sounds, "toggle expand Song Changed Event Settings");
