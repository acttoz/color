...
public class MasterAudioClipManager : EditorWindow {
    ...
    #else
        public AudioClipFormat bulkFormat = AudioClipFormat.PCM;
    ...
    private void ShowBulkOperations() {
		...
		#else
			bulkFormat = (AudioClipFormat)EditorGUILayout.EnumPopup(bulkFormat, GUILayout.Width(136));
		#endif
    ...
    private void DisplayClips() {
        ...
        for (var i = 0; i < FilteredClips.Count; i++) {
			...
			#else
				aClip._format = (AudioClipFormat) EditorGUILayout.EnumPopup(aClip._format, GUILayout.Width(136));
			#endif
    ...
    private void ApplyClipChanges(AudioInfo info, bool writeChanges) {
		...
		#else
			importer.sampleRate = info._compressionBitrate;
		#endif
    ...
    private bool TranslateFromXML(XmlDocument xDoc) {
        ...
        try {
            ...
            for (var i = 0; i < files.Count; i++) {
				...
				#else
					var format = (AudioClipFormat) Enum.Parse(typeof(AudioClipFormat), aNode.Attributes["format"].Value);
    ...
    private void BuildCache() {
        ...
        for (var i = 0; i < filePaths.Length; i++) {
			...
			#else
				var bitrate = importer.sampleRate;
			#endif
			...
			#else
				AudioInfo newClip = new AudioInfo(aPath, Path.GetFileNameWithoutExtension(aPath), true, bitrate, importer.forceToMono, importer.format, importer.loadType);
			#endif
    ...
    public class AudioInfo {
		...
		#else
			public float _origCompressionBitrate;
			public float _compressionBitrate;
			public AudioClipFormat _origFormat;
			public AudioClipLoadType _origLoadType;
			public AudioClipFormat _format;
        ...
        #else
        public AudioInfo(string fullPath, string name, bool is3d, float compressionBitrate, bool forceMono, AudioClipFormat format, AudioClipLoadType loadType) {
            _origIs3d = is3d;
            _origCompressionBitrate = compressionBitrate;
            _origForceMono = forceMono;
            _origFormat = format;
