...
public class SoundGroupVariationUpdater : MonoBehaviour {
    ...
    private float soundPlayFinishTime;
    ...
    private void PlaySoundAndWait() {
		...
		var clipLength = VarAudio.clip == null ? 0 : Math.Abs(VarAudio.clip.length / VarAudio.pitch);
    ...
    private void StopOrChain() {
        ...
        if (!VarAudio.loop || (playSnd.isPlaying && playSnd.isChainLoop)) {
            GrpVariation.Stop();
        }

        if (playSnd.isPlaying && playSnd.isChainLoop) {
