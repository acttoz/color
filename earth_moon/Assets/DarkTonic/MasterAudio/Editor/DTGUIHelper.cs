...
public static class DTGUIHelper {
	...
	public static float DisplayVolumeField(float vol, VolumeFieldType fieldType, float volumeMin = 0f, bool showFieldName = false, string fieldName = "Volume") {
		if (!MasterAudio.UseDbScaleForVolume) {
			switch (fieldType) {
				case VolumeFieldType.MixerGroup:
					return GUILayout.HorizontalSlider(vol, 0f, 1f, GUILayout.Width(60));
				...
				case VolumeFieldType.Bus:
					return GUILayout.HorizontalSlider(vol, 0f, 1f, GUILayout.Width(86));
				...
				case VolumeFieldType.None:
					if (showFieldName) {
						return EditorGUILayout.Slider(fieldName, vol, 0f, 1f);
					} else {
						return EditorGUILayout.Slider(vol, 0f, 1f, GUILayout.Width(252));
					}
			}
		} 
		...
		switch (fieldType) {
			case VolumeFieldType.MixerGroup:
				newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(60));
				break;
			case VolumeFieldType.DynamicMixerGroup:
				newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(100));
				break;
			case VolumeFieldType.Bus:
				newDbLevel = GUILayout.HorizontalSlider(dbLevel, MIN_DB, MAX_DB, GUILayout.Width(86));
				break;
			case VolumeFieldType.PlaylistController:
