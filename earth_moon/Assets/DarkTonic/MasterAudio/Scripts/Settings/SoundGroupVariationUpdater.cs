...
public class SoundGroupVariationUpdater : MonoBehaviour {
    ...
    private void PlaySoundAndWait() {
		...
		var clipLength = VarAudio.clip == null ? 0 : Math.Abs((VarAudio.clip.length - offset) / VarAudio.pitch);
    ...
    private void StopOrChain() {
		...
		if (!VarAudio.loop || (wasPlaying && playSnd.isChainLoop)) {
			GrpVariation.Stop();
        }

		if (wasPlaying && playSnd.isChainLoop) {
